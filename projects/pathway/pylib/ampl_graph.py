...
try:
    pio.kaleido.scope.timeout = 30  # 30-second timeout to prevent hangs on Windows
except Exception:
    pass
import numpy as np
...
class AmplGraph:
    ...
    def __init__(self, pkl_file, ampl_obj, case_study, show=False, save_pdf=False):
        self.pkl_file = pkl_file
        self.show = show        # Set True to open each graph in browser (slow)
        self.save_pdf = save_pdf  # Set True to attempt PDF export (requires working kaleido)
        self.x_axis = [2020, 2025, 2030, 2035, 2040, 2045, 2050]
        # Quebec electricity is essentially 100% renewable (hydro-dominated)
        self.re_share_elec = np.ones(len(self.x_axis))  # len now 7 (2020–2050)
    ...
    @staticmethod
    def _safe_write_image(fig, path, width=1200, height=550, timeout=30):
        """Write a plotly figure to an image file with a timeout.
        Prevents hangs caused by kaleido/Chromium issues on Windows."""
        error = [None]
        def _target():
            try:
        ...
        if t.is_alive():
            print(f"[WARNING] write_image timed out ({timeout}s) — skipping {path}")
        elif error[0]:
            print(f"[WARNING] write_image failed for {path}: {error[0]}")
    ...
    def graph_layer(self, ampl_collector = None, plot = True):
        ...
        # Use only layers that actually exist in the results
        col_plot = [c for c in results.columns if c in results.columns]
        ...
        for k in results.columns:
            ...
            if plot:
                ...
                eud_2025 = df_to_plot.loc[(df_to_plot['Elements']=='END_USES') &
                                          (df_to_plot['Years']=='2025')][k]
                eud_2050 = df_to_plot.loc[(df_to_plot['Elements']=='END_USES') &
                                          (df_to_plot['Years']=='2050')][k]
                EUD_2025 = float(eud_2025.values[0]) if len(eud_2025) > 0 else 0
                EUD_2050 = float(eud_2050.values[0]) if len(eud_2050) > 0 else 0
                ...
                if self.save_pdf: self._safe_write_image(fig,self.outdir+"Layers/"+k+".pdf", width=1200,
                                height=550)
                plt.close()
    ...
    def graph_resource(self, ampl_collector = None, plot = True):
        ...
        # (includes both Belgium and QC resource names)
        order_entry = ['ELECTRICITY','ELECTRICITY_EHV','ELECTRICITY_HV','ELECTRICITY_MV','ELECTRICITY_LV',
                       'GASOLINE','DIESEL','LFO','HFO','LNG','JETFUEL',
                       'PROPANE','COAL','AMMONIA','METHANOL','H2',
                       'GAS','NG_EHP','NG_HP','NG_MP','NG_LP',
                       'SNG_EHP','SNG_HP','SNG_MP','SNG_LP',
                       'H2_EHP','H2_HP','H2_MP','H2_LP',
                       'URANIUM','WOOD','WET_BIOMASS','WASTE','WASTE_BIO','WASTE_FOS',
                       'RES_SOLAR','RES_WIND','RES_WIND_ONSHORE','RES_WIND_OFFSHORE',
                       'RES_HYDRO','RES_GEO','RES_TIDAL',
                       'AMMONIA_RE','METHANOL_RE','H2_RE','GAS_RE','BIO_METHANOL','BIO_ETHANOL',
                       'BIODIESEL','BIO_DIESEL','BIOETHANOL','BIO_JETFUEL','BIO_LFO','BIO_HFO']
        ...
        all_resources = list(df_to_plot['Resources'].unique())
        order_entry = [x for x in order_entry if x in all_resources] + \
                      [x for x in all_resources if x not in order_entry]
        ...
        df_to_plot['Years'] = df_to_plot['Years'].str.replace('YEAR_', '')
        df_to_plot.dropna(subset=['Years','Res'], inplace=True) # To remove empty rows
        ...
        if plot:
            ...
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Resources.pdf", width=1200, height=550)
    ...
    def graph_gwp_per_sector(self, ampl_collector = None, plot = True):
        if ampl_collector is None:
            ampl_collector = self.ampl_collector
        _yb_cols = list(ampl_collector['Year_balance'].columns)
        # Use only layers that exist in Year_balance and are not CO2 bookkeeping
        _co2_candidates = ['CO2_ATM','CO2_CAPTURED','CO2_INDUSTRY',
                           'CO2_E','CO2_CS','CO2_EE']
        CO2_layers = [c for c in _co2_candidates if c in _yb_cols]
        _layer_candidates = ['GAS','SNG','H2','AMMONIA','METHANOL','ELECTRICITY',
                             'HEAT_HIGH_T','HEAT_LOW_T_DECEN','HEAT_LOW_T_DHN',
                             'HVC','MOB_FREIGHT_BOAT','MOB_FREIGHT_RAIL',
                             'MOB_FREIGHT_ROAD','MOB_PRIVATE','MOB_PUBLIC',
                             'HEAT_LOW_T']
        layers = [l for l in _layer_candidates if l in _yb_cols]
        mob_freight_layers = [l for l in ['MOB_FREIGHT_BOAT','MOB_FREIGHT_ROAD',
                              'MOB_FREIGHT_RAIL'] if l in _yb_cols]
        order_entry = ['MOB_PRIVATE','MOB_PUBLIC','MOBILITY_FREIGHT',
                       'ELECTRICITY','HEAT_HIGH_T','HEAT_LOW_T_DHN',
                       'HEAT_LOW_T_DECEN','HEAT_LOW_T','HVC','METHANOL','AMMONIA']
        ...
        try:
            ...
            # Model uses gwp_e[y, res, t] instead of gwp_op[res] — compute t_op-weighted average
            try:
                gwp_e_df = self.ampl_obj.get_elem('gwp_e', type_of_elem='Param').copy().reset_index()
                gwp_e_df.columns = ['Years', 'Elements', 'Periods', 'gwp_e']
                t_op_df = self.ampl_obj.get_elem('t_op', type_of_elem='Param').copy().reset_index()
                t_op_df.columns = ['Periods', 't_op']
                gwp_e_df = gwp_e_df.merge(t_op_df, on='Periods')
                gwp_e_df['weighted'] = gwp_e_df['gwp_e'] * gwp_e_df['t_op']
                agg = gwp_e_df.groupby(['Years', 'Elements'])[['weighted', 't_op']].sum()
                gwp_op = agg[['weighted']].rename(columns={'weighted': 'gwp_op'})
                gwp_op['gwp_op'] = gwp_op['gwp_op'] / agg['t_op']
            except Exception:
                resources = self.ampl_obj.sets['RESOURCES']
                yb = ampl_collector['Year_balance']
                years = yb.index.get_level_values('Years').unique().tolist()
                mi = pd.MultiIndex.from_product([years, resources], names=['Years','Elements'])
                gwp_op = pd.DataFrame(0.0, index=mi, columns=['gwp_op'])
        
        year_balance_full = ampl_collector['Year_balance'].copy()
        year_balance_full.dropna(how='all',inplace=True)
        co2_to_drop = [c for c in CO2_layers if c in year_balance_full.columns]
        year_balance_full.drop(columns=co2_to_drop, inplace=True)
        ...
        layers = [l for l in layers if l in total_prod.columns]
        ...
        layers_in_out.drop(columns=CO2_layers, errors='ignore', inplace=True)
        ...
        _gwp_candidates = ['GAS','SNG','H2','AMMONIA','ELECTRICITY','DIESEL',
                           'GASOLINE','METHANOL','LFO','HFO','LNG','JETFUEL',
                           'COAL','URANIUM','WOOD','WET_BIOMASS']
        update_gwp_res = [r for r in _gwp_candidates
                          if r in year_balance_full.columns]
        ...
        for res in update_gwp_res:
            ...
            temp_tech = temp.loc[temp.index.get_level_values('Elements').
                                 isin(technologies)].copy()
            temp_res.dropna(inplace=True)
            temp.update(temp_res['gwp_op'])
            gwp_op_2 = gwp_op.reset_index()
            gwp_op_2 = gwp_op_2.pivot_table(index=['Years'], columns=['Elements'],
                                             values='gwp_op', aggfunc='last')
        ...
        gwp_per_layer = pd.concat([others, mob_freight])


        gwp_op = gwp_op[~gwp_op.index.duplicated(keep='last')]
        ...
        actual_layers = set(gwp_per_layer['Layers'].unique())
        filtered_order = [x for x in order_entry if x in actual_layers]
        gwp_per_layer['Layers'] = pd.Categorical(
            gwp_per_layer['Layers'], filtered_order)
        ...
        if plot:
            ...
            # Improve and save PDF figure
            self.custom_fig(fig,title,yvals)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Gwp_per_sector.pdf", width=1200,
                            height=550)
            plt.close()
    ...
    def graph_cost(self):
        ...
        df_to_plot['Cost'] = df_to_plot['Cost']/1000 # To get [M$CAD]
        ...
        title = "<b>Annual system cost</b><br>[M$CAD/y]"
        temp = df_to_plot.groupby(['Years']).sum()
        first_year = str(self.x_axis[0])
        ref_val = round(float(temp.loc[first_year]['Cost']), 1) if first_year in temp.index else 0
        yvals = sorted([0, min(round(temp['Cost'],1)), ref_val, max(round(temp['Cost'],1))])
        
        self.custom_fig(fig,title,yvals)
        if self.save_pdf: self._safe_write_image(fig,self.outdir+"System_cost.pdf", width=1200, height=550)
        plt.close()
    ...
    def graph_cost_return(self, ampl_collector = None, plot = True):
        ...
        if plot:
            ...
            if len(df_to_plot['Years'].unique()) == 1:
                ...
                _mob = temp.loc[temp['Category'] == 'MOB_PRIVATE']['Share_return']
                mob_val = round(float(_mob.values[0]), 1) if len(_mob) > 0 else 0
                yvals = sorted([0, mob_val, max(round(temp['Share_return'],1))])
                self.custom_fig(fig,title,yvals,xvals=df_to_plot['Category'].
                                unique(),type_graph = 'bar')
                fig.update_xaxes(showticklabels=False,visible=False)
                if self.save_pdf: self._safe_write_image(fig,self.outdir+"Cost_return.pdf", width=1200,
                                height=550)
                plt.close()
            ...
            fig = px.line(df_to_plot_eff, x='Years', y = 'Cost_return',color='Category',
                         title=self.case_study + ' - Salvage value [M$CAD]',
            ...
            title = "<b>Respective salvage values, at the end year of each time window</b><br>[M$CAD]"
            _elec = df_to_plot_eff.loc[(df_to_plot_eff['Years'] == '2035') & (df_to_plot_eff['Category'] == 'ELECTRICITY')]['Cost_return']
            _inf  = df_to_plot_eff.loc[(df_to_plot_eff['Years'] == '2035') & (df_to_plot_eff['Category'] == 'INFRASTRUCTURE')]['Cost_return']
            ELEC_2035 = round(float(_elec.values[0]), 1) if len(_elec) > 0 else 0
            INF_2035  = round(float(_inf.values[0]), 1)  if len(_inf)  > 0 else 0
            yvals = sorted([0, ELEC_2035, INF_2035])
            self.custom_fig(fig,title,yvals, xvals=df_to_plot_eff['Years'].unique())
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Return_eff_line.pdf", width=1200, height=550)
            plt.close()
            ...
            title = "<b>Sum of respective salvage values, at the end year of each time window</b><br>[M$CAD]"
            _elec = df_to_plot_eff.loc[(df_to_plot_eff['Years'] == '2035') & (df_to_plot_eff['Category'] == 'ELECTRICITY')]['Cost_return']
            _inf  = df_to_plot_eff.loc[(df_to_plot_eff['Years'] == '2035') & (df_to_plot_eff['Category'] == 'INFRASTRUCTURE')]['Cost_return']
            ELEC_2035 = round(float(_elec.values[0]), 1) if len(_elec) > 0 else 0
            INF_2035  = round(float(_inf.values[0]),  1) if len(_inf)  > 0 else 0
            temp = df_to_plot_eff.groupby(['Years']).sum()['Cost_return']
            yvals = sorted([0, INF_2035, round(ELEC_2035+INF_2035,1), round(temp.iloc[-1],1), round(temp.iloc[0],1)])
            self.custom_fig(fig,title,yvals, xvals=df_to_plot_eff['Years'].unique())
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Return_eff_area.pdf", width=1200, height=550)
            plt.close()
    ...
    def graph_cost_inv_phase_tech(self, ampl_collector = None, plot=True):
        ...
        # Exclude the first phase (initialisation phase, e.g. '2020_2025')
        all_phases = sorted(df_to_plot['Phases'].unique())
        first_phase = [all_phases[0]] if all_phases else ['2020_2025']
        df_to_plot = df_to_plot.loc[~df_to_plot['Phases'].isin(first_phase)]
        df_to_plot['Technologies'] = df_to_plot['Technologies'].astype("str")
        ...
        if plot:
            ...
            title = "<b>Investments over transition</b><br>[M$CAD]"
            temp = df_to_plot_full.groupby(['Years']).sum()
            yvals = sorted([0,min(round(temp['cumsum'],1)),max(round(temp['cumsum'],1))])
            
            self.custom_fig(fig,title,yvals)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"C_inv_phase.pdf", width=1200, height=550)
            plt.close()
    ...
    def graph_cost_op_phase(self, ampl_collector = None, plot=True):
        ...
        if plot:
            ...
            title = "<b>Operation over transition</b><br>[M$CAD]"
            temp = df_to_plot.groupby(['Years']).sum()
            _nre = df_to_plot.loc[(df_to_plot['Years'] == '2050') &
                                  (df_to_plot['Category'] == 'NRE_FUELS')]['cumsum']
            _re  = df_to_plot.loc[(df_to_plot['Years'] == '2050') &
                                  (df_to_plot['Category'] == 'RE_FUELS')]['cumsum']
            NRE_2050 = float(_nre.values[0]) if len(_nre) > 0 else 0
            RE_2050  = float(_re.values[0])  if len(_re)  > 0 else 0
            yvals = sorted([0, min(round(temp['cumsum'],1)),
                            round(NRE_2050,1), round(NRE_2050+RE_2050,1),
                            max(round(temp['cumsum'],1))])
            
            self.custom_fig(fig,title,yvals)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"C_op_phase.pdf", width=1200, height=550)
            plt.close()
    ...
    def graph_total_cost_per_year(self, ampl_collector = None, plot = True):        
        ...
        fig = px.line(df_to_plot, x='Years', y = 'TotalCost',
                     title=self.case_study + ' - Total cost per year [M$CAD]')
        if plot:
            if self.show: pio.show(fig)
        if self.save_pdf: self._safe_write_image(fig,self.outdir+"Cost_total_per_year.pdf", width=1200, height=550)
        plt.close()
    ...
    def graph_tech_cap(self, ampl_collector = None, plot = True):
        ...
        for sector, tech in dict_tech.items():
            ...
            temp = results.loc[results.index.get_level_values('Technologies').isin(tech),'F_Mult']
            df_to_plot = pd.DataFrame(index=temp.index,columns=['F_Mult'])
            ...
            if plot:
                if len(df_to_plot.index.get_level_values(0).unique()) <= 1:
                    fig = px.bar(df_to_plot, x='Years', y = 'F_Mult',color='Technologies',
                                 title=self.case_study + ' - ' + sector+' - Installed capacity',
                                 color_discrete_map=self.color_dict_full)
                else:
                    
                    fig = px.area(df_to_plot, x='Years', y = 'F_Mult',color='Technologies',
                ...
                if len(df_to_plot.index.get_level_values(0).unique()) >= 1:
                    ...
                    temp = df_to_plot.groupby(['Years']).sum()
                    yvals = sorted([0,round(min(temp['F_Mult']),1),round(max(temp['F_Mult']),1)])
                    
                    self.custom_fig(fig,title,yvals,xvals=sorted(df_to_plot['Years'].unique()))
                        
                    
                    if self.save_pdf: self._safe_write_image(fig,self.outdir+"Tech_Cap/"+sector+".pdf", width=1200, height=550)
                plt.close()
    ...
    def graph_load_factor(self):
        ...
        for sector, tech in dict_tech.items():
            temp = results.loc[results.index.get_level_values('Technologies').isin(tech),['F_Mult','F_year']]
            temp.dropna(how='all',inplace=True)
            temp['Load_factor'] = temp['F_year']/(8760*temp['F_Mult'])
    ...
    def graph_load_factor_scaled(self, ampl_collector = None, plot = True):
        ...
        results = ampl_collector['Assets'].copy()
        dict_tech = self.ampl_obj.sets.get('TECHNOLOGIES_OF_END_USES_TYPE', {})
        ...
        for sector, tech in dict_tech.items():
            temp = results.loc[results.index.get_level_values('Technologies').
                               isin(tech),['F_Mult','F_year']]
            temp.dropna(how='all',inplace=True)
            temp['Load_factor_scaled'] = temp['F_year']/(8760*temp['F_Mult'])

            df_to_plot = pd.DataFrame(index=temp.index,
                                      columns=['F_Mult','F_year','Load_factor_scaled'])
            df_to_plot.update(temp)
            ...
            df_to_plot = pd.melt(df_to_plot.reset_index(), id_vars=['Years', 'Technologies'],
                                 value_vars=['F_Mult', 'F_year','Load_factor_scaled'], var_name='var',
                                 value_name='val')
            df_to_plot_full[sector] = df_to_plot 
            if plot and not df_to_plot.empty:
                ...
                plt.close()
        df_unused = df_unused.loc[~df_unused['F_Mult'].isna()]
    ...
    def graph_comparison(self,output_files,type_of_graph):
        ...
        if type_of_graph in ['C_op_phase','C_inv_phase_tech']:
            ...
            if type_of_graph == 'C_op_phase':
                title = "<b>Cumulative opex difference versus REF</b><br>[M$CAD]"
            else:
                title = "<b>Cumulative capex difference versus REF</b><br>[M$CAD]"
            yvals = sorted([round(min(df_to_plot['cumsum']),1),0,
                     round(max(df_to_plot['cumsum']),1)])
            
            self.custom_fig(fig,title,yvals,neg_value=True)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"{}_diff_PF.pdf".format(type_of_graph), width=1200, height=550)
            ...
            for sector, tech in dict_tech.items():
                ...
                df_temp = pd.DataFrame(0,index=mi_temp,columns=['F_Mult'])
                ...
                fig = px.line(df_to_plot_s,x='Years',y='F_Mult',color='Technologies',
                ...
                title = "<b>Installed capacities difference versus REF - {}</b><br>[{}]".format(sector,
                                                                                                self.dict_tech_cap_unit[sector])
                yvals = sorted([round(min(df_to_plot_s['F_Mult']),1),0,
                         round(max(df_to_plot_s['F_Mult']),1)])
                ...
                if self.save_pdf: self._safe_write_image(fig,self.outdir+"Tech_Cap/Diff_PF/{}.pdf".format(sector), width=1200, height=550)
                plt.close()
            ...
            for k in df_to_plot.keys():
                ...
                if self.save_pdf: self._safe_write_image(fig,self.outdir+"Layers/Diff_PF/{}.pdf".format(k), width=1200, height=550)
                plt.close()
            ...
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"GWP_per_sector_Diff_PF.pdf", width=1200, height=550)
            plt.close()
            ...
            for sector in df_to_plot.keys():
                ...
                if len(df_to_plot[sector].index.get_level_values(0).unique()) >= 1:
                    ...
                    if self.save_pdf: self._safe_write_image(fig,self.outdir+"Load_factor_scaled/Diff_PF/"+sector+".pdf", width=1200, height=550)
                plt.close()
            ...
            self.custom_fig(fig,title,yvals,neg_value=True)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Primary_res_diff_PF.pdf", width=1200, height=550)
            plt.close()
            ...
            self.custom_fig(fig,title,yvals,neg_value=True)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Primary_res_diff_PF_category.pdf", width=1200, height=550)
            plt.close()
            ...
            fig.update_xaxes(categoryorder='array', categoryarray= sorted(df_to_plot['Years'].unique()))
            if self.show: pio.show(fig)
            title = "<b>Cost return difference versus REF</b><br>[M$CAD]"
            ...
            self.custom_fig(fig,title,yvals,neg_value=True)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Cost_return_diff_PF.pdf", width=1200, height=550)
            plt.close()
        elif type_of_graph in ['Total_trans_cost']:
            ...
            fig.update_xaxes(categoryorder='array', categoryarray= sorted(df_to_plot['Years'].unique()))
            if self.show: pio.show(fig)
            title = "<b>Cumulative transition total cost difference versus REF</b><br>[M$CAD]"
            yvals = sorted([min(round(df_to_plot['Tot_trans_cost'],1)),0,
                     max(round(df_to_plot['Tot_trans_cost'],1))])
            self.custom_fig(fig,title,yvals,neg_value=True)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Cum_total_cost_diff_REF.pdf", width=1200, height=550)
            plt.close()
            ...
            fig.update_xaxes(categoryorder='array', categoryarray= sorted(df_to_plot['Years'].unique()))
            if self.show: pio.show(fig)
            title = "<b>Annual system cost difference versus REF</b><br>[M$CAD]"
            yvals = sorted([min(round(df_to_plot['TotalCost'],1)),0,
                            round(df_to_plot['TotalCost'].iloc[-1],1),
                     max(round(df_to_plot['TotalCost'],1))])
            self.custom_fig(fig,title,yvals,neg_value=True)
            if self.save_pdf: self._safe_write_image(fig,self.outdir+"Cum_system_cost_diff_REF.pdf", width=1200, height=550)
            plt.close()
    ...
    def _group_tech_per_eud(self):
        ...
        non_energy_types = self.ampl_obj.sets.get('END_USES_TYPES_OF_CATEGORY', {}).get('NON_ENERGY', [])
        for ned in non_energy_types:
    ...
    def _group_sets(self):
        ...
        re_resources_key = 'RE_RESOURCES' if 'RE_RESOURCES' in self.ampl_obj.sets else 'RENEW'
        categories['RE_FUELS'] = self.ampl_obj.sets[re_resources_key].copy()
        categories['INFRASTRUCTURE'] = self.ampl_obj.sets['INFRASTRUCTURE'].copy()
        re_fuels = self.ampl_obj.sets[re_resources_key].copy()
        ...
        tech_of_eut = self.ampl_obj.sets.get('TECHNOLOGIES_OF_END_USES_TYPE', {})
        for k in categories:
            for j in categories[k]:
                if k == 'HEAT_LOW_T':
                    dhn_techs = tech_of_eut.get("HEAT_LOW_T_DHN", [])
                    if j in dhn_techs:
                        categories_2[j] = 'HEAT_LOW_T_DHN'
                    else:
                        categories_2[j] = 'HEAT_LOW_T_DECEN'
                elif k == 'MOBILITY_PASSENGER':
                    pub_techs = tech_of_eut.get("MOB_PUBLIC", [])
                    if j in pub_techs:
    ...
    def _dict_layer_unit(self):
        # Derive layers dynamically from Year_balance columns (excluding non-layer cols)
        yb = self.ampl_collector.get('Year_balance', None)
        if yb is not None:
            layers = [c for c in yb.columns if c not in ('Year_balance',)]
        else:
        ...
        mob_pass = ['MOB_PRIVATE','MOB_PUBLIC']
        mob_freight = ['MOB_FREIGHT_BOAT','MOB_FREIGHT_RAIL','MOB_FREIGHT_ROAD',
                       'MOB_FREIGHT_BOAT_DIESEL','MOB_FREIGHT_ROAD_DIESEL','MOB_FREIGHT_RAIL_DIESEL']
        dict_layer_unit = {}
        for sector in layers:
            if sector in mob_pass:
                dict_layer_unit[sector] = 'Gpass.-km'
            elif sector in mob_freight or sector.startswith('MOB_FREIGHT'):
    ...
    def dict_color(category):
        ...
        if category == 'Electricity':
            ...
            color_dict = {"ELECTRICITY" : "deepskyblue", "GASOLINE" : "gray", "DIESEL" : "silver", "BIOETHANOL" : "mediumorchid", "BIODIESEL" : "mediumpurple", "LFO" : "darkviolet", "GAS" : "orange", "GAS_RE" : "gold", "WOOD" : "saddlebrown", "WET_BIOMASS" : "seagreen", "COAL" : "black", "URANIUM" : "deeppink", "WASTE" : "olive", "H2" : "violet", "H2_RE" : "plum", "AMMONIA" : "slateblue", "AMMONIA_RE" : "blue", "METHANOL" : "orchid", "METHANOL_RE" : "mediumvioletred", "CO2_EMISSIONS" : "gainsboro", "RES_WIND" : "limegreen", "RES_SOLAR" : "yellow", "RES_HYDRO" : "blue", "RES_GEO" : "firebrick", "ELEC_EXPORT" : "chartreuse","CO2_ATM": "dimgray", "CO2_INDUSTRY": "darkgrey", "CO2_CAPTURED": "lightslategrey", "RE_FUELS": 'green','NRE_FUELS':'black', 'LOCAL_RE': 'limegreen', 'IMPORTED_ELECTRICITY': 'deepskyblue',
                          ...
                          "WASTE_BIO": "yellowgreen", "WASTE_FOS": "dimgray",
                          "RES_WIND_ONSHORE": "lawngreen", "RES_WIND_OFFSHORE": "forestgreen", "RES_TIDAL": "teal",
                          "BIO_DIESEL": "darkolivegreen", "BIO_METHANOL": "mediumseagreen",
                          "BIO_ETHANOL": "lightgreen", "BIO_JETFUEL": "palegreen",
                          "BIO_LFO": "olivedrab", "BIO_HFO": "darkolivegreen",
                          "HFO": "slategray", "LNG": "steelblue", "JETFUEL": "lightsteelblue", "PROPANE": "lightyellow",
                          # QC electricity and gas pressure levels
                          "ELECTRICITY_EHV": "dodgerblue", "ELECTRICITY_HV": "deepskyblue",
                          "ELECTRICITY_MV": "lightskyblue", "ELECTRICITY_LV": "lightcyan",
                          "NG_EHP": "darkorange", "NG_HP": "orange", "NG_MP": "moccasin", "NG_LP": "bisque",
                          "SNG_EHP": "goldenrod", "SNG_HP": "gold", "SNG_MP": "khaki", "SNG_LP": "lightyellow",
                          "H2_EHP": "violet", "H2_HP": "mediumpurple", "H2_MP": "plum", "H2_LP": "lavender"}
    ...
    def custom_fig(fig,title,yvals,xvals=['2025','2030','2035','2040',
        ...
        try:
            ...
            fig.layout.xaxis.tickvals = sorted(fig.layout.xaxis.tickvals, key=str)
        try:
            fig.layout.xaxis.range = sorted(fig.layout.xaxis.range)
        except TypeError:
            fig.layout.xaxis.range = sorted(fig.layout.xaxis.range, key=str)
