...
def run_pathway_materials(
    ...
    materials_recycling : bool
        If True, loads Material_recycling.dat (recycling_rate from the RR_*
        sheets, regenerated by run_build_rr.py) -- the technical ceiling on
        how much of a material can be recycled. Without this, no recycling
        happens at all (recycling_rate defaults to 0).
    follow_scenario : bool
        Only meaningful when materials_recycling=True. If True, forces the
        model to recycle EXACTLY recycling_scenario_share[y,mat] (Recycling_scenario
        sheet -- currently dummy 0.1->0.7 ramp values, clipped per material to
        its own achievable recycling_rate by rr_build_table) of each
        material's decommissioned amount, aggregated across recyclable
        technologies (recycled_material_scenario equality in Constraints.mod)
        -- an equality, not a minimum, so the optimizer loses the freedom to
        recycle more even when it would otherwise want to. If False (default),
        the optimizer decides freely how much to recycle, bounded only by
        recycling_rate's ceiling and the material limits (no cost
        differential exists yet between recycling and disposal beyond the
        default disposal_cost).
    materials_recycling_process : bool
        ...
        Fully independent from materials_recycling/follow_scenario (own
    ...
    for i in range(len(ampl_pre.years_opti)):
        ...
        if follow_scenario:
            ampl.set_params('follow_scenario', 1)
