...
# Nette de Recycled_material, agregee par materiau (fongible entre technos)
subject to material_content_year_limit {y in YEARS_WND diff YEAR_ONE, mat in MATERIALS}:
    sum {tec in MATERIAL_TECHS} Material_content_year[y,tec,mat]
    - sum {tec in MATERIAL_TECHS} (Recycled_material[y,tec,mat] + Recycled_material_process_total[y,tec,mat]) <= limit_material_year[y,mat];

subject to material_content_limit {mat in MATERIALS}:
    sum {tec in MATERIAL_TECHS} Material_content[tec,mat]
    - sum {y in YEARS_WND diff YEAR_ONE, tec in MATERIAL_TECHS} (Recycled_material[y,tec,mat] + Recycled_material_process_total[y,tec,mat]) * 5 <= limit_material[mat];
...
subject to recycled_material_objective {y in YEARS_WND diff YEAR_ONE, mat in MATERIALS}:
    follow_objective * (sum {tec in MATERIAL_TECHS : recycling_rate[y,tec,mat] > 0} Recycled_material[y,tec,mat] + Recycling_shortfall[y,mat])
    >= follow_objective * recycling_objective_share[y,mat] * sum {tec in MATERIAL_TECHS : recycling_rate[y,tec,mat] > 0} Decommissioned_material[y,tec,mat];

# Alimente l'objectif (PES_obj_pathway.mod), pas C_material -- garde TotalTransitionCost non faussee.
...
unfix C_material;  # PES_main.mod le fixe a 0 par defaut ; libere ici pour que l'egalite ci-dessous le pilote
