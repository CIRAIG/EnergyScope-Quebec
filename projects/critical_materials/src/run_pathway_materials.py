...
def run_pathway_materials(
    ...
    materials_recycling_process : bool
        If True (in addition to materials_recycling=True), also loads
        Material_recycling_process.dat -- Approach 2, competing recycling
        processes (currently PV c-Si module/infrastructure only, see
        rt_pipeline). Compare against a materials_recycling_process=False run
        to see Approach 2's effect vs Approach 1 (simple recycling_rate)
        alone.
    ...
    if(materials_recycling):
        mod_2_path.append(str(pth_materials / 'Material_recycling.dat'))  # recycling_rate, regenerated by run_build_rr.py from Recycling_rates.xlsx

    if(materials_recycling_process):
        mod_2_path.append(str(pth_materials / 'Material_recycling_process.dat'))  # Approach 2 (competing processes), regenerated by run_build_rt.py
    ...
    for i in range(len(ampl_pre.years_opti)):
        ...
        for var_name in ('Material_content_year', 'Decommissioned_material', 'Recycled_material', 'Disposed_material'):
            ...
            if var_name == 'Recycled_material':
                # Indexed by (Years,Technologies,Materials,RECYCLING_PROCESS) in AMPL (competing
                # recycling processes, cf. Constraints.mod) -- summed over process here to keep the
                # same (Years,Technologies,Materials) shape as every other materials_results entry.
                # Use results['Recycled_material_by_process'] (below) for the per-process breakdown.
                df.index.names = ['Years', 'Technologies', 'Materials', 'RECYCLING_PROCESS']
                by_process = df.loc[df.index.get_level_values('Years').isin(curr_years_wnd), :]
                if materials_results.get('Recycled_material_by_process') is None:
                    materials_results['Recycled_material_by_process'] = by_process
                else:
                    combined = pd.concat([materials_results['Recycled_material_by_process'], by_process])
                    materials_results['Recycled_material_by_process'] = combined.loc[~combined.index.duplicated(keep='last')].sort_index()
                df = df.groupby(level=['Years', 'Technologies', 'Materials']).sum()
            else:
