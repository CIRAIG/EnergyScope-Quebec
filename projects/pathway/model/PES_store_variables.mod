...
set SET_INIT_SOL := {"F_up_to",	"F_new_up_to",	"F_decom_up_to",	"F_old_up_to",	"F_used_year_start_next", "Res_up_to", "Res_t_up_to"};
...
var F_decom_up_to {PHASE_UP_TO,PHASE_UP_TO union {"2015_2020","2020_2025"}, TECHNOLOGIES} >= 0; #[GW] Accounts for the decommissioned capacity in a new phase from the start of the optimisation (2020)
subject to store_F_decom_up_to {p_decom in PHASE_UP_TO, p_built in PHASE_UP_TO union {"2015_2020","2020_2025"}, j in TECHNOLOGIES}:
...
subject to store_Res_up_to {j in RESOURCES, y in YEARS_UP_TO}:
	Res_up_to[y,j] = Res[y,j]; # Reference Res (fixed for past years), not F_Mult_t sum (free)

var Res_t_up_to {YEARS_UP_TO, RESOURCES, PERIODS} >= 0; # Per-period resource use for cost calc
subject to store_Res_t_up_to {j in RESOURCES, y in YEARS_UP_TO, t in PERIODS}:
	Res_t_up_to[y,j,t] = Res_t [y,j,t]; # Reference Res_t (fixed for past years), not F_Mult_t (free)
