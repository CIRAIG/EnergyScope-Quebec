...
class AmplPreProcessor:
    ...
    def remaining_update(self,i,file_in = 'PES_data_remaining.dat', file_out = 'PES_data_remaining_wnd.dat'):
        ...
        # Read header to get phase column indices by name
        with open(pth_file, encoding='utf-8') as fp:
            header = re.split(r'\s+', fp.readline().strip())
            # header looks like: ['param', 'remaining_years', ':', '2015_2020', ..., ':=']
            all_phases_in_file = [h for h in header if h not in ('param','remaining_years',':',':=','')]
            col_idx = {phase: i+1 for i, phase in enumerate(all_phases_in_file)}  # +1 for tech name col
            l = fp.readlines()
            n = len(re.split(r'\t+', l[0].rstrip('\t'))) if l else 0
            ...
            with open(pth_file,encoding='utf-8') as fp:
                ...
                for line in fp:
                    ...
                    if len(list_line) < n:
                        ...
                        f.write(list_line[0]+'\t')
                        for phase in phase_list:
                            idx = col_idx.get(phase)
                            if idx is not None and idx < len(list_line):
                                f.write(list_line[idx]+'\t')
                        f.write('\n')
            f.write('; \n')
    ...
    def without_2015_2020(self):
        ...
        # YEAR_2020 kept: optimization now starts at 2020
        self.list_phase.remove('2015_2020')
        # 2020_2025 kept: first optimization phase
