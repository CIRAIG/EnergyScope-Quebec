...
Generates per-year QC_techs_<year>.dat and QC_eud_<year>.dat files
from snapshot model results and prospective ratio CSVs.
...
def _build_snapshot_model(snapshot_dir: Path, validation_dat: str) -> Model:
    """Assemble a snapshot Model from the given directory and validation dat file.
    ...
    return Model([
        ...
        ('dat', snapshot_dir / validation_dat),
    ])
...
def _build_params_df(res) -> pd.DataFrame:
    """Extract all tech parameters from snapshot results into a flat DataFrame.
            ...
            for i in range(len(df_tech)):
                ...
                if param in ['c_p', 'c_p_t', 'lifetime', 'ref_size', 'trl']:
                    ...
                    if param != 'c_p_t':
                        if abs(value) < _ZERO_THR:
                            continue
                        f.write(f"let {param}['YEAR_{year}','{tech}'] := {value} ; \n")
                    else:
                        period = df_tech['index1'].iloc[i]
                        if value != 1:
                            if abs(value) < _ZERO_THR:
                                continue
                            f.write(f"let {param}['YEAR_{year}','{tech}',{period}] := {value} ; \n")
                    ...
                    if flow == tech:
                        if abs(value) < _ZERO_THR:
                            continue
                        f.write(f"let {param}['YEAR_{year}','{tech}','{flow}'] := {value} ; \n")
                        continue
                    ...
                    if abs(value * prospective_ratio) < _ZERO_THR:
                        continue
                    f.write(f"let {param}['YEAR_{year}','{tech}','{flow}'] := {value}*{prospective_ratio} ; \n")
                    ...
                    if abs(value * prospective_ratio) < _ZERO_THR:
                        continue
                    if param == 'c_op':
                        f.write(f"let {param}['YEAR_{year}','{tech}',{df_tech['index1'].iloc[i]}] := {value}*{prospective_ratio} ; \n")
                    else:
                        f.write(f"let {param}['YEAR_{year}','{tech}'] := {value}*{prospective_ratio} ; \n")
...
def run(snapshot_dir: Path, prospective_dir: Path,
        output_techs_dir: Path, output_eud_dir: Path, scenario: str,
        validation_2021: str = 'QC_validation_2021.dat',
        validation_2023: str = 'QC_validation_2023.dat') -> None:
    """Generate all per-year QC_techs_<year>.dat and QC_eud_<year>.dat files.
