...
def _create_sankey_figure(df_flow: pd.DataFrame, colors: Colors, long_names: bool=False) -> go.Figure:
    """
    ...
    node = np.unique(np.concatenate((df_flow['source' if not long_names else 'source (long)'].unique(), df_flow['target' if not long_names else 'target (long)'].unique())))
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        link=dict(
            source=df_sankey['source' if not long_names else 'source (long)'],
            target=df_sankey['target' if not long_names else 'target (long)'],
            value=df_sankey['value'],
            label=df_sankey['value'],
            color=df_sankey['color'].apply(lambda c: Color(c).rgba(opacity))
        )
    )])
    fig.update_layout(title_text="Sankey diagram of energy flows (GWh/year)", font_size=10, template="plotly", font_color="black")
...
def generate_sankey_flows(results: Result, aggregate_mobility, aggregate_grid, aggregate_technology,
    ...
    EUD_heat = EUD.loc[(EUD['source'].str.contains('HEAT_HIGH_T'))
                       ...
                       | (EUD['source'].str.contains('HEAT_LOW_T_SH')), :]
